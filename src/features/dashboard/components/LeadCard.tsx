    ...
    return (
        <button
            ...
            className={`w-full text-left p-5 rounded-2xl border transition-all duration-300 flex items-center gap-5 relative group ${isActive
                ? 'bg-foreground/5 border-foreground/40 shadow-sm ring-1 ring-foreground/10 translate-x-1'
                : 'bg-card border-border hover:border-foreground/20 hover:bg-muted/30'
                }`}
        >
            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-[15px] font-black shrink-0 transition-all duration-300 ${isActive 
                ? 'bg-foreground text-background shadow-soft' 
                : 'bg-muted text-text-muted group-hover:bg-foreground/10 group-hover:text-foreground'
                }`}>
                {number < 10 ? `0${number}` : number}
            </div>

            <div className="flex-1 min-w-0">
                <h4 className="text-[16px] font-bold text-foreground leading-tight truncate group-hover:text-foreground transition-colors">{name}</h4>
                ...
                <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-3">
                    <div className="flex items-center gap-1.5 text-text-secondary">
                        ...
                        <span className="text-[14px] font-bold opacity-80">{mobile}</span>
                    </div>
                    {email && (
                        <div className="flex items-center gap-1.5 text-text-secondary truncate">
                            ...
                            <span className="text-[14px] font-bold opacity-80 truncate max-w-[120px]">{email}</span>
                        </div>
                    )}
                </div>
            </div>
            ...
            {isActive && (
                <div className="absolute right-4 w-1.5 h-10 bg-foreground rounded-full" />
            )}
        </button>
    );
}
