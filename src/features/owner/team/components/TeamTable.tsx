                        ...
                        <table className="w-full text-left border-collapse min-w-[800px] lg:min-w-0">
                            <thead>
                                <tr className="bg-foreground/10 border-b border-border">
                                    ...
                                    <th className="p-3 sm:p-4 text-[14px] font-bold text-adaptive uppercase tracking-widest">Full Name</th>
                                    <th className="p-3 sm:p-4 text-[14px] font-bold text-adaptive uppercase tracking-widest">Contact</th>
                                    <th className="p-3 sm:p-4 text-[14px] font-bold text-adaptive uppercase tracking-widest text-center">Experience</th>
                                    <th className="p-3 sm:p-4 text-[14px] font-bold text-adaptive uppercase tracking-widest">Emirates ID</th>
                                    <th className="p-3 sm:p-4 text-[14px] font-bold text-adaptive uppercase tracking-widest">Bank Details</th>
                                ...
                                {members.length > 0 ? (
                                    members.map((member, index) => (
                                        <tr key={member.id} className="hover:bg-muted/30 transition-colors group">
                                            ...
                                            <td className="p-3 sm:p-4">
                                                <div>
                                                    <p className="text-sm sm:text-sm font-medium text-adaptive leading-tight">{member.name}</p>
                                                    <p className="text-[11px] text-text-muted mt-0.5">Joined: {member.created_at ? new Date(member.created_at).toLocaleDateString() : '-'}</p>
                                                </div>
                                            </td>
                                            <td className="p-3 sm:p-4">
                                                <div className="space-y-0.5">
                                                    <p className="text-sm sm:text-sm font-medium text-adaptive">{member.email}</p>
                                                    <p className="text-[11px] text-text-muted">{member.phone}</p>
                                                </div>
                                            </td>
                                            <td className="p-3 sm:p-4 text-center">
                                                <span className="px-3 py-1 bg-foreground/10 text-foreground rounded-lg text-sm font-bold border border-foreground/10 whitespace-nowrap">
                                                    {member.experience} Years
                                                </span>
                                            </td>
                                            <td className="p-3 sm:p-4">
                                                <p className="text-sm font-bold text-text-secondary font-mono">{member.emirates_id}</p>
                                            </td>
                                            <td className="p-3 sm:p-4">
                                                <div className="space-y-0.5">
                                                    <p className="text-sm sm:text-sm font-medium text-adaptive">{member.bank_name || '-'}</p>
                                                    <p className="text-[11px] text-text-muted font-mono">{member.iban || '-'}</p>
                                                </div>
                                            </td>
